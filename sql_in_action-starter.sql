...
-- EXPECTED OUTPUT: 146
-- SELECT id, count(*) AS TotalRows FROM intro_sql.final_airbnb;


-- <<<<<<<<<<<<<<<<<<<<<< PROBLEM 2 >>>>>>>>>>>>>>>>>>>>>>>
-- Find out the name of the host for "host_id" 63613
-- HINT: "Where" could it be?
-- SELECT host_name, host_id FROM intro_sql.final_airbnb WHERE host_id = 63613;
...
-- SELECT distinct neighbourhood AS distinct_neighbourgoods FROM intro_sql.final_airbnb;
...
-- SELECT id, number_of_reviews FROM intro_sql.final_airbnb WHERE room_type = 'Private room' ORDER BY number_of_reviews desc Limit 1;
...
-- SELECT neighbourhood, count(neighbourhood) AS popularity_neighbourhood FROM intro_sql.final_airbnb group by neighbourhood order by popularity_neighbourhood desc limit 1;
-- EXPECTED OUTPUT: Williamsburg
-- INVESTIGATE: Should Williamsburg be crowned the most popular neighbourhood?
 -- Harlem is also the most popular
...
-- SELECT * FROM intro_sql.final_airbnb WHERE minimum_nights < 7 order by reviews_per_month desc limit 1;
-- EXPECTED OUTPUT: 58059
...
-- SELECT host_name, COUNT(host_id) AS Listings FROM intro_sql.final_airbnb GROUP BY host_name ORDER BY Listings desc limit 1;
...
-- SELECT AVG(price) as average_price  FROM intro_sql.final_airbnb;
-- SELECT host_name, price, ((price / 165.1) * 10) as Percentage FROM intro_sql.final_airbnb;
